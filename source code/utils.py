...
def visualise_cql_q_table(q_table):
    print("Central Q-table (Joint Actions):")
    # Joint actions: (0,0), (0,1), (1,0), (1,1)
    actions_map = {0: "C", 1: "D"}
    for a1 in range(2):
        for a2 in range(2):
            joint_action = (a1, a2)
            # Find the index of this joint action
            # We know it's a1 * 2 + a2 if we ordered them that way, 
            # but let's be safe and assume a simple index if we knew the mapping.
            # In my CQL implementation, joint_actions = list(itertools.product(range(2), range(2)))
            # so (0,0)->0, (0,1)->1, (1,0)->2, (1,1)->3
            idx = a1 * 2 + a2
            val = q_table[str(((0, 0), idx))]
            print(f"Q({actions_map[a1]}, {actions_map[a2]}) = {val:.2f}")
    print()


def visualise_evaluation_returns(means, stds, savefig=None):
    """
