...
class IQL:
    ...
    def learn(
        ...
        **IMPLEMENT THIS FUNCTION**
        ...
        """
		# Algorithm 2 from M4-4_MARL in Games.pdf
        for i in range(self.num_agents):
            key = str((obss[i], actions[i]))
            current_q = self.q_tables[i][key]

            if done:
                max_next_q = 0
            else:
                max_next_q = max([self.q_tables[i][str((n_obss[i], a))] for a in range(self.n_acts[i])])

            target = rewards[i] + self.gamma * max_next_q
            self.q_tables[i][key] += self.learning_rate * (target - current_q)
